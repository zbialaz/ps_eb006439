...
class DAO
{
    ...
    /**
     * Método SET para acesso direto via nomes de propriedades da classe
     *
     * @param string $name Nome da propriedade
     * @return mixed $value Valor a ser inserido
     * @return mixed
     */
    ...
    /**
     * Função que objetiva retornar as metainformações da classe, baseando-se para isso na leitura dos Attributes
     *
     * @return array Propriedades da entidade (tabela e campos)
     */
    public function getTableInfo() : array
    {
        // vetor que armazenará as informações da classe referente às tabelas e campos do banco de dados
        $info = [];
        ...
        foreach($ref->getAttributes(Entidade::class) as $attrTable) {
            ...
            foreach($ref->getProperties() as $propriedade) {
                // pra cada campo/propriedade localizada, procura seus atributos
    ...
    /**
     * Função genérica que busca dados do banco de dados para a entidade relacionada ao objeto instanciado
     *
     * @param array $params Os parâmetros de condição da busca (where), ex: ['titulo ='=>'Teste']
    ...
    public function loadById(int|string $id) : bool
    {
        ...
        // ALIMENTAR O OBJETO, PROPRIEDADE POR PROPRIEDADE A PARTIR DESTE VETOR RETORNADO
    ...
    /**
     * Método que, com todos os atributos obrigatórios alimentados salva os dados no banco de dados
     * se houver ID informado, então atualiza um registro, se não, cria
     *
     * @return void
     */
    public function save() : bool
    {
        ...
        foreach($this->getFields() as $atributo => $parametros) {
            ...
            // verificar se o campo atual está nulo e é um not nul (NN)
            if (is_null($this->$atributo) && array_key_exists('nn', $parametros)) {
                $label = $parametros['label'];
                throw new Exception("O campo {$label} deve ser preenchido");
            }

            $campos[$atributo] = $this->$atributo;
        }
        
        // se não tiver valor no atributo do campo chave será feito um insert, pois não é um registro conhecido
